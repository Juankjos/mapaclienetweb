...
function setActionsEnabled(id, on){
    document.querySelectorAll('#'+id+' .btn').forEach(b => setEnabled(b, on));
}
function showActions(id, show){
    const el = document.getElementById(id);
    if (!el) return;
    el.classList.toggle('show', !!show);     // transición
    setActionsEnabled(id, !!show);           // habilitar/deshabilitar botones
}
...
document.querySelector('[data-action="save-nombre"]').addEventListener('click', async ()=>{
    ...
    if (nuevo.length < 2) { Swal.fire('Nombre demasiado corto','Escribe al menos 2 caracteres.','warning'); return; }
